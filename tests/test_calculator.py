...
def test_total_cost_calculation():
    ...
    server1 = IonosServer(
        "server1", {"cores": 2, "ram": 4, "volume": {"size": 50, "type": "SSD"}}, pricing
    )

    server2 = IonosServer(
        "server2", {"cores": 4, "ram": 8, "volume": {"size": 100, "type": "SSD"}}, pricing
    )
...
def test_cost_by_resource_type():
    ...
    server2 = IonosServer(
        "server2", {"cores": 2, "ram": 4, "volume": {"size": 50, "type": "SSD"}}, pricing
    )
...
def test_detailed_breakdown():
    ...
    server = IonosServer(
        "test-server", {"cores": 2, "ram": 4, "volume": {"size": 50, "type": "SSD"}}, pricing
    )
