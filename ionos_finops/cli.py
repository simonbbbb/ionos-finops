...
@click.option(
    "--plan-file",
    type=click.Path(exists=True),
    help="Path to Terraform plan file (.tfplan)"
)
@click.option(
    "--format",
...
@click.option(
    "--use-api",
    is_flag=True,
    help="Use IONOS API for real-time pricing"
)
@click.option(
    "--api-token",
    envvar="IONOS_TOKEN",
    help="IONOS API token"
)
@click.option(
    "--cache-ttl",
    type=int,
    default=24,
    help="Pricing cache TTL in hours"
)
def breakdown(path: str, plan_file: Optional[str], format: str, output: Optional[str], region: str, 
              config: Optional[str], use_api: bool, api_token: Optional[str], cache_ttl: int):
    try:
        custom_pricing = None
        if config:
...
def scheduler(api_token: Optional[str], interval: int, status: bool, stop: bool):
    try:
        from ionos_finops.pricing.scheduler import get_scheduler, start_global_scheduler, stop_global_scheduler
